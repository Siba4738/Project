...
# Config MySQL
app.config['MYSQL_HOST'] = 'containers-us-west-139.railway.app'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = '6KuJUBi7G7YLqtByhyCi'
app.config['MYSQL_PORT'] = 6100
...
def login():
    if request.method == 'POST':
        ...
        print("\n\n!!!! Things are are::", username, password_candidate)

        # Create cursor
        cur = mysql.connection.cursor()
        print("\n\n!!!! Cursor connection done")

        # Get user by username
        result = cur.execute("SELECT * FROM admins WHERE username = %s", [username])
        print("\n\n!!!! Cursor executed command")
